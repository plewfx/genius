        ...
        <div className="flex items-center justify-between px-2 py-2">
            <form action="" className="flex items-center lg:bg-white px-2 py-1">
                <div className={cn("lg:hidden mr-3", isOpenSearch && "hidden")}>
                    {isOpenMenu ? (
                        <X size={20} className="cursor-pointer" onClick={() => setIsOpenMenu(false)} />
                    ) : (
                        <Menu size={20} className="cursor-pointer" onClick={() => setIsOpenMenu(true)} />
                    )}
                </div>

                <input
                placeholder="Search lyrics & more"
                className={cn("placeholder:text-black lg:placeholder:focus:text-gray-500 placeholder:focus:text-black/30 focus:outline-0 text-base placeholder:text-sm font-semibold bg-primary lg:bg-white", !isOpenSearch && "hidden lg:block")}
                />
                
                <Search className={cn("", isOpenSearch && "hidden lg:block")} width={20} height={20} onClick={() => setIsOpenSearch(true)} />
            </form>
            ...
            <X className={cn("lg:hidden", !isOpenSearch && "hidden")} onClick={() => setIsOpenSearch(false)} />
        </div>

        <div className={cn("flex items-center flex-col lg:flex-row lg:justify-center gap-10 bg-gray-900 py-3 px-[10%]", isOpenMenu ? "max-lg:h-[100vh] max-lg:pt-10" : "max-lg:hidden")}>
            <ul className="flex items-center flex-col lg:flex-row gap-10">
                {navigation.map((item, index) => (
                    <li key={index} className={cn("text-white text-sm font-medium tracking-widest uppercase", index + 1 < navigation.length && "lg:border-r lg:pr-10")}>
                        <Link href={item.url}>
                            {item.text}
                        </Link>
                    </li>
            ...
            <ul className="lg:hidden xl:flex flex items-center gap-5">
                {socialLinks.map((item, index) => (
                    <li key={index} className="text-white text-sm font-medium tracking-widest uppercase">
                        <Link href={item.url}>
                            {<item.icon />}
                        </Link>
                    </li>
